...
CheckUp:
	...
	movle	r1, #2					// Store 2 (NONE) in vertical direction
	strleb	r1, [r3, #3]
...
CheckLeft:
	...
	bgt		UpdateVelocity			// If greater than 0 we exit since mario isnt done changing direction
...
CheckRight:
	...
	bne		UpdateVelocity// Set H Di to 2?
	ldr		r3, =MarioState	
...
CheckAButton:	
	...
	ldrb	r1, [r3, #3]			// Vertical Direction byte; 2 == NONE
	cmp		r1, #2					// If 2, we will process jump command, else we are mid animation
	bne		UpdateVelocity			// Skip jump command if mid animation
...
UpDown:
	...
	cmp		r1, #9					// Check his offset from base
	bge		SlowerA					// If its 8 or more, movement is slower

	ldrb	r5, [r3, #4]

	cmp		r0, #0					// Check Direction
	addeq	r2, r5					// If 0 (down) add 12 px from y coord
	subeq	r1, #1					// Decrement vertical offset from base
	subne	r2, r5					// If 1 (up) subtract 12 px from y coord
...
SlowerA:
	...
	cmp		r0, #0					// Check Direction
	addeq	r2, r5					// If 0 (down) add 8 px to y coord
	subeq	r1, #1					// Decrement vertical offset from base
	subne	r2, r5					// If 1 (up) subtract 8 px from y coord
	addne	r1, #1					// Increment vertical offset from base
	str		r2, [r3, #12]			// Leftmost y coord
	strb	r1, [r3, #6]
...
SlowerB:
	...
	cmp		r0, #0					// Check Direction
	addeq	r2, r5					// If 0 (down) add 4 px from y coord
	subeq	r1, #1					// Decrement vertical offset from base
	subne	r2, r5					// If 1 (up) subtract 4 px from y coord
	addne	r1, #1					// Increment vertical offset from base
	str		r2, [r3, #12]			// Leftmost y coord
	strb	r1, [r3, #6]
